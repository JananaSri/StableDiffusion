...
def update_prompt(prompt, artist_name_opt, photo_style_opt, art_styles_choices,
    ...
    if len(art_styles_choices) > 0 and art_styles_choices[0] != 'None':
        prompt += ", " + ", ".join(art_styles_choices)
...
def get_file_info_in_json(image_file_name):
    ...
    with open(json_file_path) as json_file_data:
        ...
        with self.ui_obj:
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("Generate Image > Text Prompt"):
                    with gr.Row():
                        with gr.Column():
                            audio_prompt = gr.Audio(label="Microphone Input", type="filepath", sources=["microphone"])

                            audio_prompt_button = gr.Button("Create Prompt from Audio Input")
                            text_input = gr.Textbox(lines=3, placeholder="Prompt Here...")

                            art_styles_choices = gr.CheckboxGroup(choices=self.art_styles, label="Artist Choices", value=[])
